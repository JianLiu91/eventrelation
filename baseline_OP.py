...
def produce_output(inputf, outfile):
    ...
    print('event mentions', event_mentions)
    print('event pairs', event_pairs)

    # temp = {'0': []}
    # for k1 in event_mentions:
    #     for k2 in event_mentions:
    #         if k1 != k2:
    #             temp['0'].append((k1, k2))
    # print(temp)
    # event_pairs = temp
...
#    print(event_mentions)
    for k, v in event_pairs.items():
        for i in v:
            ...
            #output.writelines("_".join(event_mentions[i[1]]) + "\t" + "_".join(event_mentions[i[0]]) + "\tPRECONDITION"  + "\n")
            output.close()
